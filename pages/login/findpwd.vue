	...
	<view>
		<view class="main-body">
			...
			<view class="input-flex">
				...
				<input autocomplete="off" type="text" class="input-flex-text" name="yzm" v-model="yzm"  >
				<view class="input-flex-btn" v-bind:class="yzmClass" @click="getYzm()">{{yzmStatus}}</view>
			</view>
			<view class="input-flex">
				<view class="input-flex-label">新密码</view>
				<input autoComplete="off" class="input-flex-text" name="password" type="text" password=true  placeholder="请填写密码">
			</view>
	...
	export default {
		...
		methods:{
			...
			getYzm:function(){
					...
					var that=this;
					that.app.get({
						url:that.app.apiHost+"/login/sendsms",
						data:{
			...
			formSubmit:function(e){
				var that=this;
				that.app.post({
					url:that.app.apiHost+"/login/findpwdsave",
					unLogin:true,
					data:e.detail.value,
					success:function(res){
						uni.showToast({

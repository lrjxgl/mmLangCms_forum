<template>
	<view v-if="pageLoad">
		<view v-if="error"  class="emptyData">很抱歉，内容已下架...</view>
		<view v-else>
			<view class="main-body pd-5 bg-fff">
				...
				<view class="row-box">
					...
					<div class="flex-col flex-center mgb-5">
						<image style="width: 100%;" @longpress="downImg(item)" class="wmax mgb-5" mode="widthFix"
							v-for="(item,key) in imgsList" :key="key" :src="item.trueimgurl"></image>
					</div>
